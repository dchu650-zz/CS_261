...
public class Sudoku 
{
	 ...
	 * @return
	 */
	public boolean checkCol3x3(int row, int col, int num, int last)
	{
		if(col < last)
		{
			if(numberList[row][col] != num)
				return checkCol3x3(row, col+1, num, last);
			else
				return false;
		}
		return true;
	...
	public boolean checkBox(int row, int col, int num)
	{
		...
		if(checkCol3x3(yValue, xValue, num, row) == false) //check first col in box if false
			return false;
		if(checkCol3x3(yValue + 1, xValue, num, row) == false) //check the second col in box if false
			return false;
		if(checkCol3x3(yValue + 2, xValue, num, row) == false) //check the third col in box if false
			return false;
		return true;
	}
